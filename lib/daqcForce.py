...
class daqcForce:
    def __init__(self,root,addr,channel):
        #self.scale = Scale(samples=2)
        self.scale=HX711(5,6)

        self.scale.reset()
        self.scale.setReferenceUnit(3280) # todo, make this a config
        self.scale.tare()
        ...
        self.var.set(1)
        self.val=DoubleVar()
        self.val.set(self.scale.getWeight())
        ...
        self.var.set(1)
        self.a2dl = StringVar(root, value="Force Channel "+str(self.chan)+":")
